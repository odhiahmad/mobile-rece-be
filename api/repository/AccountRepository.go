...
func (a *accountRepository) LoginByUsername(dtoLogin *dto.Login) (string, error) {
	...
	if err = a.db.Debug().Table("accounts").Where("username = ?", dtoLogin.Username).Preload(clause.Associations).Find(&accounts).Error; err != nil {
		return "", err
	}
...
func (a *accountRepository) ForgotPassword(email string) error {

	var err error
	if err = a.db.Debug().Table("accounts").Where("email = ?", email).Error; err != nil {
		return err
	}
